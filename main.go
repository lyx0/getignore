...
func main() {
	...
	// Parse the language flag.
	// If no flag has been provided the pLang string will contain "default".
	flag.StringVar(&pLang, "lang", "default", "Programming language")
	flag.Parse()

	// If the parsed value is not the default one, check if it is in our language map.
	if strings.ToLower(pLang) != "default" {
		// Some flag was provided.
		for name, url := range languages {
			// Check if it is a key value.
		...
		// No language flag has been provided so we warn the user and show the usage.
		fmt.Println("---------------------------------------------")
		fmt.Println("WARNING: No language was provided.")
		fmt.Println(`Usage: getignore --lang="go"`)
		fmt.Println(`Language codes can be found here: https://github.com/github/gitignore`)
		fmt.Println("---------------------------------------------")
		return
	}
